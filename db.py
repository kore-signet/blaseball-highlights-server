...
class HighlightDB:
    def __init__(self, app: Quart, db: str) -> None:
        self.init_app(app)
        self._pool = None
        self.db_name = db
    ...
    async def _before_serving(self) -> None:
        self._pool = await asyncpg.create_pool(database=self.db_name)
    ...
    async def create_story(self, story, events, user = None):
        ...
        async with self._pool.acquire() as conn:
            while True:
                ...
                row = await conn.fetchrow("SELECT * FROM stories WHERE story_id = $1",story_id)
                if not row:
            ...
            await conn.execute('''
                ...
                VALUES ($1,$2,$3,$4)
            ''', story["story_id"], uuid.UUID(story["game_id"]), user["user_id"], story["title"])
        ...
        if not user_existed:
            return {"status": 200, "user_token": user["user_token"], "user_id": user["user_id"], "story_id": story["story_id"]}
        else:
            return {"status": 200, "story_id": story["story_id"]}

    async def edit_story(self, story, events, user = None):
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM stories WHERE story_id = $1", story["story_id"])
            if not row:
                return {"status": 404, "reason": "story id not found"}

            if row["user_id"] != user["user_id"] or not (await self.check_user_token(user, conn)):
    ...
    async def get_story(self,id):
        async with self._pool.acquire() as conn:
            ...
            for event in (await conn.fetch("SELECT * FROM events WHERE story_id = $1", id)):
                res["events"].append({
    ...
    async def check_user_token(self, user, conn):
        row = await conn.fetchrow("SELECT * FROM users WHERE user_id = $1 AND user_token = $2",user["user_id"],user["user_token"])
        return (row != None)
